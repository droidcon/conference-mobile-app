...
import static novoda.droidcon2010uk.util.UIUtils.buildStyledSnippet;
import static novoda.droidcon2010uk.util.UIUtils.formatSessionSubtitle;
 ...
 */
public class SessionsActivity extends ListActivity implements
		AsyncQueryListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mTrackUri = intent
				.getParcelableExtra(SessionDetailActivity.EXTRA_TRACK);
		...
		if (!Sessions.isSearchUri(sessionsUri)) {
			...
			sortOrder = Sessions.DEFAULT_SORT;
			mColorizeTitle = mTrackUri == null; // only colourize title if not in track view with color title bar 
		} else {
			mAdapter = new SearchAdapter(this);
			projection = SearchQuery.PROJECTION;
			sortOrder = Sessions.SEARCH_SORT;
			mColorizeTitle = true;
		}
		...
		mHandler.startQuery(sessionsUri, projection, sortOrder);
	}
	...
	private class SessionsAdapter extends CursorAdapter {
		...
		public View newView(Context context, Cursor cursor, ViewGroup parent) {
			return getLayoutInflater().inflate(R.layout.list_item_session,
					parent, false);
		}
		...
		public void bindView(View view, Context context, Cursor cursor) {
			...
			final String subtitle = formatSessionSubtitle(blockStart, blockEnd,
					roomName, context);
			...
			// Possibly indicate that the session has occurred in the past, and what track it belongs to
			UIUtils.setSessionTitleColor(blockStart, blockEnd, titleView,
					subtitleView, mColorizeTitle, cursor
					.getString(SessionsQuery.SESSION_ID), getContentResolver());
	...
	private interface SessionsQuery {
		String[] PROJECTION = { BaseColumns._ID, Sessions.SESSION_ID,
				Sessions.TITLE, Sessions.STARRED, Blocks.BLOCK_START,
				Blocks.BLOCK_END, Rooms.ROOM_NAME };
	...
	private interface SearchQuery {
		String[] PROJECTION = { BaseColumns._ID, Sessions.SESSION_ID,
				Sessions.TITLE, Sessions.SEARCH_SNIPPET, Sessions.STARRED, };
